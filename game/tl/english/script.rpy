# TODO: Translation updated at 2024-01-04 20:38

translate english strings:

    # game/script.rpy:31
    old "{size=-12}Prologue:{/size}\nAmanecer descolorido"
    new "{size=-12}Prologue:{/size}\nFaded Dawn"

    # game/script.rpy:41
    old "Prólogo: Amanecer descolorido"
    new "Prologue: Faded Dawn"

    # game/script.rpy:41
    old "Jugando..."
    new "Playing..."

    # game/script.rpy:63
    old "Está versión del juego es solo una demo\nGracias por jugar Lack Of Colors\nNo olvides dar tu opinión sobre el mismo\nMe ayudarás mucho con tu comentario\n:)"
    new "This version of the game is just a demo\Thank you for playing Lack Of Colors\nDon't forget to give your opinion about it\nYou will help me a lot with your comment\n:)"
# TODO: Translation updated at 2024-01-05 21:26

translate english strings:

    # game/script.rpy:31
    old "{size=-12}Prólogo:{/size}\nAmanecer descolorido"
    new "{size=-12}Prologue:{/size}\nFaded Dawn"

